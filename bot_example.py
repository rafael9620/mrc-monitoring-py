...
ID = 1268569363  # A establecer por el usuario (consultar mediante @get_id_bot)
...
def start(bot, update):
    ...
    update.message.reply_text("Este es un Bot que permite controlar y comprobar ciertos aspectos del sistema Orange Pi en el que se aloja. Para conocer los comandos implementados consulta la /ayuda")
...
def apagar(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        # Respondemos al comando con el mensaje
        update.message.reply_text("Apagando el sistema")
...
def reiniciar(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        # Respondemos al comando con el mensaje
        update.message.reply_text("Reiniciando el sistema")
...
def temp(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        # Llamada al sistema
        temp = llamadaSistema("cat /etc/armbianmonitor/datasources/soctemp")
...
def almacenamientos(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        ...
        # Respondemos al comando con el mensaje
        update.message.reply_text(_fdisk)
...
def arquitectura(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        ...
        # Respondemos al comando con el mensaje
        update.message.reply_text(_arquitectura)
...
def kernel(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        ...
        # Respondemos al comando con el mensaje
        update.message.reply_text(_kernel)
...
def drivers(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        ...
        # Respondemos al comando con el mensaje
        update.message.reply_text(_lsmod)
...
def lsusb(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        ...
        # Respondemos al comando con el mensaje
        update.message.reply_text(_lsusb)
...
def borrar(bot, update, args):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        if len(args) == 1:  # Comprobar si el comando presenta argumento o no
            ...
            # Respondemos al comando con el mensaje
            update.message.reply_text("Archivo " + archivo + " borrado")
        else:
            # Respondemos al comando con el mensaje
            update.message.reply_text(
                "Especifica un archivo.\n\nEjemplo:\n/borrar /home/user/archivo.txt")
...
def ssh_on(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        ...
        # Respondemos al comando con el mensaje
        update.message.reply_text("Iniciando servidor SSH")
...
def ssh_off(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        ...
        # Respondemos al comando con el mensaje
        update.message.reply_text("Deteniendo servidor SSH")
...
def ssh_estado(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        respuesta = llamadaSistema(
            "/etc/init.d/ssh status")  # Llamada al sistema
        # Respondemos al comando con el mensaje
        update.message.reply_text(respuesta)
...
def vnc_on(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        # Respondemos al comando con el mensaje
        update.message.reply_text("Iniciando servidor VNC")
        # Llamada al sistema
        llamadaSistema(
...
def vnc_off(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        # Respondemos al comando con el mensaje
        update.message.reply_text("Cerrando servidor VNC")
...
def scriptfex(bot, update):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        # Transforma Script.bin en Script.fex
        llamadaSistema("bin2fex /boot/script.bin /boot/script.fex")
        # Esperamos 5 segundos (para que se complete la transformacion)
        time.sleep(5)
        bot.sendDocument(ID, open('/boot/script.fex', 'rb')
                         )  # Enviamos el archivo
...
def exportar(bot, update, args):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        if len(args) == 1:  # Solo hacer caso si el comando presenta argumento
            ...
            # Respondemos al comando con el mensaje
            update.message.reply_text(
                "Se debe especificar el archivo que deseas extraer.\n\nEjemplo:\n/exportar /home/user/archivo")
...
def importar(bot, update, args):
    ...
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        if len(args) == 1:  # Solo hacer caso si el comando presenta argumento
            ...
            update.message.reply_text(
                "Inserta el archivo a enviar (tipo documento)")
            esperando_archivo = 1
        else:
            # Respondemos al comando con el mensaje
            update.message.reply_text(
                "Se debe especificar la ruta donde deseas importar el archivo.\n\nEjemplo:\n/importar /home/user")
    ...
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        if len(args) == 2:  # Si el comando presenta 2 argumentos
            ...
            if esperando_ruta == 0:
                enlace_descarga = args[0]
                update.message.reply_text(
                    "Especifica a continuacion, la ruta donde almacenar el archivo a descargar.\n\nEjemplo:\n/descargar https://raw.githubusercontent.com/J-Rios/TelegramBots/master/opibot.py /home/usuario/descargas")
                esperando_ruta = 1
        else:
            # Respondemos al comando con el mensaje
            update.message.reply_text(
                "Se debe especificar el enlace (URL) de descarga y el directorio de descarga.\n\nEjemplo:\n/descargar https://raw.githubusercontent.com/J-Rios/TelegramBots/master/opibot.py /home/usuario/descargas")
...
def buscar(bot, update, args):
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        if len(args) == 2:  # Si el comando presenta 2 argumentos
            ...
            ruta = args[1]
            resultado = llamadaSistema(
                "find " + ruta + " -name '*" + nombre_archivo + "*'")  # Llamada al sistema
            # Respondemos al comando con el mensaje
            update.message.reply_text(
                "Archivos encontrados para el termino de busqueda:\n\n'" + resultado)
        else:
            # Respondemos al comando con el mensaje
            update.message.reply_text(
                "Se debe especificar el archivo y el directorio desde el que buscar.\n\nEjemplo:\n/buscar .log /")
...
def archivo_recibido(bot, update):
    ...
    if update.message.chat_id == ID:  # Solo hacer caso si quien le habla es el remitente correspondiente a dicha ID
        if esperando_archivo == 1:
            ...
            os.chdir(ruta_poner_archivo)
            update.message.reply_text(
                "Archivo " + nombre_archivo + " recibido y posicionado en " + ruta_poner_archivo)
            esperando_archivo = 0
