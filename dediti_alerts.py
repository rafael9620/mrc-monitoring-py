...
def elastichealth():
	...
	msg = bell + " <b>Elasticsearch</b> (<i>{}</i>)\n       <b>cluster: </b> {}\n       <b> status: </b> {}\n       <b>  nodes: </b> {}"
...
def apihealth():
	critical = False
	bell = emojize(":bell:", use_aliases=True)
	msg = bell + " <b>Dediti Backend</b> (<i>{}</i>)\n       <b>elasticsearch: </b> {}\n       <b>            mysql: </b> {}"
...
def resources():
	part_alternation_mark = emojize(":part_alternation_mark:", use_aliases=True)
	...
	cpu = "    " + small_blue_diamond + " <b>CPU: </b>" + str(psutil.cpu_percent()) + " %"
        ram = "    " + small_blue_diamond + " <b>RAM: </b>" + str(psutil.virtual_memory().percent) + " %"
        hdd = "    " + small_blue_diamond + " <b>HDD (/):</b>"
        hto = "         " + black_small_square + " total: " + str(hdd_.total / (2**30)) + " GiB"
        hus = "         " + black_small_square + " usado: " + str(hdd_.used / (2**30)) + " GiB"
        hfr = "         " + black_small_square + " libre: " + str(hdd_.free / (2**30)) + " GiB"
	message = resources + "\n\n" + cpu + "\n" + ram+ "\n" + hdd + "\n" + hto + "\n" + hus + "\n" + hfr
	return message
...
def stats():
	...
	black_small_square = emojize(":black_small_square:", use_aliases=True)
	small_blue_diamond = emojize(":small_blue_diamond:", use_aliases=True)
	db = MySQLdb.connect("dbprod.sacavix.com","mailu","4HUaM9pF9OF2","offersapi")
	...
	data = cursor.fetchone()
	categories = "    " + small_blue_diamond + " <b>Categories:</b> " + str(data[0])
	cursor.execute("SELECT count(*) FROM offersapi.currency;")
	data = cursor.fetchone()
	currency = "    " + small_blue_diamond + " <b>Currencies:</b> " + str(data[0])
...
def operations():
	...
	stats = chart_with_upwards_trend + " <b>APIs STATS</b> <i>(last " + str(last_minutes) + " minutes)</i> "
	
	cursor = db.cursor()
	cursor.execute("select count(*) FROM operation WHERE operation.created_at > (now() - interval " + str(last_minutes) + " minute ) ;")
	data = cursor.fetchone()
	operations = "   " + small_blue_diamond + " <b>Operations:</b> " + str(data[0])
	
	cursor.execute("SELECT avg(operation.response_time) FROM offersapi.operation WHERE operation.created_at > (now() - interval " + str(last_minutes) + " minute );")
	data = cursor.fetchone()
	if(data[0] is None):
		data="-"
	else:
	   data = str(round(data[0],2)) + "s"
	time_response_avg = "   " + small_blue_diamond + " <b>Time Response Avg:</b> " + str(data)
	...
	cursor = db.cursor()
	cursor.execute("SELECT count(*) FROM offersapi.operation WHERE operation.method='POST' AND operation.created_at > (now() - interval " + str(last_minutes) + " minute ) group by method;")
	data = cursor.fetchone()
	if data is None:
		data = 0
	else:
	...
	cursor = db.cursor()
	cursor.execute("SELECT count(*) FROM offersapi.operation WHERE operation.method='GET' AND operation.created_at > (now() - interval " + str(last_minutes) + " minute ) group by method;")
	data = cursor.fetchone()
	if data is None:
		data = 0
	else:
	...
	cursor = db.cursor()
	cursor.execute("SELECT count(*) FROM offersapi.operation WHERE operation.method='PUT' AND operation.created_at > (now() - interval " + str(last_minutes) + " minute ) group by method;")
	data = cursor.fetchone()
	if data is None:
		data = 0
	else:
	...
	cursor = db.cursor()
	cursor.execute("SELECT count(*) FROM offersapi.operation WHERE operation.method='DELETE' AND operation.created_at > (now() - interval " + str(last_minutes) + " minute ) group by method;")
	data = cursor.fetchone()
	if data is None:
		data = 0
	else:
	...
	cursor = db.cursor()
	cursor.execute("SELECT count(*) FROM offersapi.operation WHERE operation.method='OPTIONS' AND operation.created_at > (now() - interval " + str(last_minutes) + " minute ) group by method;")
	data = cursor.fetchone()
	if data is None:
		data = 0
	else:
